...
def send_to_robot(posicio):
    ...
    s.connect(('84.88.129.200', 2004)) # Replace this for the Robot IP and Port that you have defined
...
def getStatusRobot():
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.connect(('127.0.0.1', 2004)) # Replace this for the Robot IP and Port that you have defined
    # Codifies the data to be send as 6 big-endian floats woth 4bytes each
    closed=False
    while not closed:
        try:
